...
exports.runTransaction = functions.https.onCall(async (data, context) => {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    //一連の読み取りと書き込み操作を
    //一つの作業単位として扱う
    const myFieldsRef = db.collection('users').doc(data.myUid); //dataは引数で与えられたdata（クライアントから渡されたデータ内容）
    ...
    if (myFieldsData.matched_status === true) { //DocumentCへのlock待機後の再readで、既にマッチング済みだった場合は、実行中のトランザクションを失敗させる
